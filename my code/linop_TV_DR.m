% Diagonal Down-Right DTV operator
%
% Author: Richard Yip
% Email: rbyip1@gmail.com

function op = linop_TV_DR( sz, variation, action )
...
if strcmpi(action,'matrix') || strcmpi(action,'cvx')
    disp('take care of this in the linop_TV_DR');
    switch lower(variation)
        case 'regular'
            ...
            I = eye(n2);
            Dr = kron(I,J);  % vertical differences, sparse matrix
        case 'circular'
            e = ones(max(n1,n2),1);
            e2 = e;
%             e2(n1:end) = 0;
...
switch lower(variation)
    case 'regular'
        Dr     = @(X) vec(DRdiffs(X)); %CHANGED
        diff_r = @(X)  [zeros(n1,1),[zeros(1,n2-1);X(1:end-1,1:end-1)]] - [[X(1:end-1,1:end-1),zeros(n1-1,1)];zeros(1,n2)]; %CHANGED
    case 'circular'
        % For circular version, 2 x 2 case is special.
%         error('not yet implemented');
        disp('GOD PLEASE HELP US');
        Dr     = @(X) vec( [diff(X,1,1); X(1,:) - X(end,:) ] );
        % diff_v needs to be checked
        diff_r = @(X) [X(end,:);X(1:end-1,:)] - X;

    otherwise
        error('Bad variation parameter');
end
if iscell(sz)
    Dr_transpose = @(X)      diff_r(mat(X))  ;
else
    Dr_transpose = @(X) vec( diff_r(mat(X)) );
end

TV  = @(x) ( Dr(mat(x)) );     % real to complex
TVt = @(z) ( Dr_transpose((z)) );
